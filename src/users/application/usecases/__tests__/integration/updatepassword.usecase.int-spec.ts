...
describe('UpdatePasswordUseCase Integration tests', () => {
  ...
  it('should throws error when a entity found', async () => {
    ...
    ).rejects.toThrow(new NotFoundError(`UserModel not found ID ${entity.id}`));
  });

  it('should throws error when old password no provided', async () => {
    const entity = new UserEntity(userDataBuilder({}));
  ...
  it('should throws error when new password no provided', async () => {
    const entity = new UserEntity(userDataBuilder({}));
    const models = await prismaService.user.create({
      data: entity.toJSON(),
    });
